...
class MachineType(models.TextChoices):
    ...
    UV_FLATBED = "UV_FLATBED", _("UV Flatbed Printer")
...
class Machine(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["company", "machine_type", "name"], name="unique_machine_per_company_type_name"
            )
        ]
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            machine_slug = slugify(self.name)
            name_slug = slugify(self.machine_type)
            self.slug = f"{company_slug}-{machine_slug}-{name_slug}"
